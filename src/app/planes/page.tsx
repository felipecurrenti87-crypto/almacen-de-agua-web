      ...
      <section className="relative pb-20 sm:pb-32">
        <div className="max-w-4xl mx-auto px-4 sm:px-6">
          ...
          <AnimatedSection delay={300}>
            <div className="glass-dark rounded-3xl p-6 sm:p-10 md:p-12 border border-celeste-neon/10">
              {/* Price display */}
              <div className="text-center mb-8 sm:mb-12">
                <AnimatePresence mode="wait">
                  ...
                  >
                    <span className="text-5xl sm:text-6xl md:text-7xl font-bold font-heading gradient-text-glow">
                      {formatPrice(precio)}
                    </span>
                  </motion.div>
                </AnimatePresence>
                <p className="text-gris-dark text-sm sm:text-base mt-2">por mes</p>
              </div>
              ...
              {!isRed && (
                <div className="mb-8 sm:mb-12">
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-sm text-gris-dark font-heading">Bidones por mes</span>
                    <span className="text-2xl sm:text-3xl font-bold font-heading text-celeste-neon">
                      {bidones}
                    </span>
                  </div>
                  <input
                    ...
                    className="w-full h-2 rounded-full appearance-none cursor-pointer bg-negro-medium
                      [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-6 [&::-webkit-slider-thumb]:h-6 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-celeste-neon [&::-webkit-slider-thumb]:shadow-[0_0_20px_rgba(56,189,248,0.5)] [&::-webkit-slider-thumb]:cursor-pointer [&::-webkit-slider-thumb]:transition-shadow [&::-webkit-slider-thumb]:hover:shadow-[0_0_30px_rgba(56,189,248,0.7)]
                      [&::-moz-range-thumb]:w-6 [&::-moz-range-thumb]:h-6 [&::-moz-range-thumb]:rounded-full [&::-moz-range-thumb]:bg-celeste-neon [&::-moz-range-thumb]:border-none [&::-moz-range-thumb]:shadow-[0_0_20px_rgba(56,189,248,0.5)] [&::-moz-range-thumb]:cursor-pointer"
                    style={{
                      background: `linear-gradient(to right, #38BDF8 0%, #38BDF8 ${((bidones - minBidones) / (maxBidones - minBidones)) * 100}%, #1C1C2E ${((bidones - minBidones) / (maxBidones - minBidones)) * 100}%, #1C1C2E 100%)`,
                    }}
                  />
                  <div className="flex justify-between text-xs text-gris-dark/60 mt-2 font-heading">
                    <span>{minBidones}</span>
                    <span>{maxBidones}</span>
                  </div>
                </div>
              )}
              ...
              {isRed && (
                <div className="text-center mb-8 sm:mb-12">
                  <div className="inline-flex items-center gap-3 bg-celeste-neon/10 rounded-2xl px-6 py-4 border border-celeste-neon/20">
                    <svg className="w-6 h-6 text-celeste-neon" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" />
                    </svg>
                    <span className="text-celeste-glow font-heading font-bold text-lg">
              ...
              {/* Price breakdown */}
              <div className="bg-negro-light/50 rounded-2xl p-4 sm:p-6 mb-8 border border-celeste-neon/5">
                <h4 className="font-heading font-bold text-sm text-gris-dark mb-3 uppercase tracking-wider">
                  Detalle
                </h4>
                <div className="space-y-2">
                  {mode === "hogar" && (
                    <>
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="text-white font-semibold">{formatPrice(precio)}</span>
                      </div>
                    </>
                  )}
                  {mode === "comercio" && comercioType === "bidon" && (
                    <>
                      ...
                      <div className="border-t border-celeste-neon/10 pt-2 flex justify-between text-sm">
                        <span className="text-celeste-glow font-semibold">Total</span>
                        <span className="text-celeste-neon font-bold">{formatPrice(precio)}</span>
                      </div>
                    </>
                  )}
                  {isRed && (
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-celeste-neon font-bold">{formatPrice(PRECIO_RED)}</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                <a
                  ...
                  className="flex-1 liquid-glass-btn flex items-center justify-center gap-2 px-8 py-4 rounded-2xl font-heading font-bold text-white text-base sm:text-lg"
                >
                ...
                <Link
                  ...
                  className="flex-1 liquid-glass-btn-outline flex items-center justify-center gap-2 px-8 py-4 rounded-2xl font-heading font-bold text-celeste-glow text-base sm:text-lg"
                >
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                {beneficiosComunes.map((b, i) => (
                  <motion.div
                    ...
                    viewport={{ once: true }}
                    className="glass-dark rounded-2xl p-4 sm:p-6 text-center border border-celeste-neon/10 hover:border-celeste-neon/25 transition-colors"
                  >
                    <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-celeste-neon/10 flex items-center justify-center text-celeste-neon mx-auto mb-3">
